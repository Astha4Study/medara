...
export interface FormEditJamOperasionalProps {
    value: JamOperasional;
    onChange: (value: JamOperasional) => void;
}

const FormEditJamOperasional: React.FC<FormEditJamOperasionalProps> = ({
    value,
    onChange,
}) => {
    const openDaysCount = value.filter((jam) => !jam.tutup).length;
    const isLastOpenDay = (index: number) =>
        !value[index].tutup && openDaysCount === 1;
        ...
        <div className="p-6">
            ...
            <div className="space-y-3">
                {value.map((jam, index) => {
                    ...
                    const lastOpenDay = isLastOpenDay(index);

                    return (
                        <div
                            key={jam.hari}
                            className={`rounded-xl border p-4 ${isTutup ? 'bg-gray-50' : 'bg-white'} ${lastOpenDay ? 'border-emerald-200' : ''}`}
                        >
                            <div className="flex flex-col gap-4 md:flex-row md:items-center">
                                ...
                                <div className="flex items-center gap-2 md:w-24">
                                    <input
                                        ...
                                        disabled={lastOpenDay}
                                    />
                                    <span
                                        className={`text-sm ${lastOpenDay ? 'text-gray-400' : 'text-gray-700'}`}
                                    >
                                        Tutup
                                    </span>
                                </div>
